...
# )
# 
# performance::check_model(global_model_2_bm_2)
# 
# AICc(global_model_2_bm, global_model_2_bm_2)
...
model_set_bm$weight <- as.numeric(model_set_bm$weight)
glimpse(model_set_bm)

# write_csv(model_set_bm , "tables/model_set_output_bm_stability.csv")

###########################################################################
# run models with delta AICc <4 -------------------------------------------
###########################################################################
m1 <- glmmTMB(bm_stability ~ latitude + max_ss + mean_bm + spp_rich + SppInvSimpDivInd + (1|project), data = model_data_scaled,
              family = gaussian(link = "log"),
              REML = FALSE)
...
pred_SppInvSimpDivInd <- ggeffect(m1, "SppInvSimpDivInd[0:3 by=0.1]", type = "re")

plot(pred_spp_rich)
plot(pred_max_ss)
plot(pred_SppInvSimpDivInd)
plot(pred_mean_bm)
plot(pred_latitude)
...
sd(model_data$SppInvSimpDivInd)#3.760947

ggplot(pred_spp_rich, aes(x = x*5.452757, y = predicted)) + 
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) +
  labs(x = "Species Richness", y = "Nutrient Supply Stability (1/CV Nutrient Supply)") +
  theme_classic() +
...
# ggsave(
#   filename = "max_ss_bm_stability_marginaleffect.tiff",
#   path = "plots",
#   width = 10, height = 10
# )

ggplot(pred_SppInvSimpDivInd, aes(x = x*3.760947, y = predicted)) + 
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) +
  labs(x = "Species Inverse Simpson Diversity Index", y = "Nutrient Supply Stability (1/CV Nutrient Supply)") +
  theme_classic() +
  theme(panel.background = element_rect(fill = "white"),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
...
# ggsave(
#   filename = "spp_invsimpdiv_bm_stability_marginaleffect.tiff",
#   path = "plots",
#   width = 10, height = 10
# )
...
# 
# # ggsave(
# #   filename = "biome_bm_stability_marginaleffect.tiff",
# #   path = "plots",
# #   width = 15, height = 10
# # )
...
# ggsave(
#   filename = "latitude_bm_stability_marginaleffect.tiff",
#   path = "plots",
#   width = 10, height = 10
# )
