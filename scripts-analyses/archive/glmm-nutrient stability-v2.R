...
###########################################################################
m1 <- glmmTMB(n_stability ~ latitude + max_ss + spp_rich + (1|project), data = model_data_scaled,
              family = gaussian(link = "log"),
              REML = FALSE)
performance::check_model(m1)

m2 <- glmmTMB(n_stability ~ latitude + max_ss + spp_rich + TrophInvSimpDivInd + (1|project), data = model_data_scaled,
             family = gaussian(link = "log"),
             REML = FALSE)
performance::check_model(m2)

m3 <- glmmTMB(n_stability ~ latitude + ecosystem + max_ss + spp_rich + TrophInvSimpDivInd + (1|project), data = model_data_scaled,
             family = gaussian(link = "log"),
             REML = FALSE)
performance::check_model(m3)

m4 <- glmmTMB(n_stability ~ 1 + (1|project), data = model_data_scaled,
              family = gaussian(link = "log"),
              REML = FALSE)
performance::check_model(m4)

# m5 <- glmmTMB(n_stability ~ project, data = model_data_scaled,
#               family = gaussian(link = "log"),
#               REML = FALSE)
