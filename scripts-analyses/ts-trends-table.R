...
model_dt <- dt1 |> 
  ...
  group_by(project, ecosystem, vert, habitat, year) |> 
...
###########################################################################
dat_ts <- dat |> 
  group_by(axis_name_5, ecosystem_2, habitat, vert) |> 
  mutate(count = n()) |> 
  ungroup() |> 
  ...
  filter(count > 10) |> 
  # group_by(vert) |> 
  group_by(axis_name_5, ecosystem_2, vert) |>
...
ar_results <- dat_ts |> 
  group_by(ecosystem_2, axis_name_5, vert, habitat) |> 
...
pdat <- ar_trends |> 
  ...
  select(ecosystem_2, axis_name_5, habitat, vert, everything()) |> 
  select(-group) |>
  rename(Ecosystem = ecosystem_2,
         LTERProgram = axis_name_5,
         Habitat = habitat,
         Community = vert,
         NutrientSupplyTrend = trend,
         TrendSE = trend_se,
         TrendPValue = trend_pval)

write_csv(pdat, "output/ms first round/tables/program-habitat-trends-table.csv")