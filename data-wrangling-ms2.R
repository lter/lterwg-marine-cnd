...
dt_mutate <- dt_og |> 
  ...
  # ditching code below since we are grouping at species level in next big code chunk for AS
...
unique(dt_mutate_filter$project)
...
dt_total <- dt_mutate_filter |> 
  group_by(project, habitat, vert, year, month, site, subsite_level1, subsite_level2, subsite_level3, scientific_name) |> 
  mutate(### calculate total phosphorus supply at each sampling unit and then sum to get column with all totals
    ...
    ### calculate average community size metrics - MOVED DOWN IN THIS SCRIPT GIVEN SPECIES GROUPING HERE
